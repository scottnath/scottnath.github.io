...
gulp.task('sass', configuration.gulpTasks.gulpSass.dependencies, function () {
  'use strict';
log.info('configuration.gulpTasks.gulpSass.src',configuration.gulpTasks.gulpSass.src);
...
gulp.task('build', function (callback) {
  ...
  runSequence(
    ...
    'glob-inject-js-all',
    function () {
      log.info('Import of files into jekyll complete');
      callback();
    }
  );
});
...
gulp.task('watch', function () {
  ...
  */
  ...
  // Global Assets SASS
  watch(configuration.fileTypes.sass.jekyllSrc, function () {
    log.info('Jekyll sass file change');
    runSequence(
      'sass',
      ['jekyll-rebuild']
    );
  ...
  if (configuration.templates) {
    configuration.templates.forEach(function (template) {
      watch(template.src, ['tpl-copy-' + template.name, browserSync.reload]);
    });
  }
});
